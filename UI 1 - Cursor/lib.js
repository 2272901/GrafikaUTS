export class CanvasHandler {
    ...
    kotak(x0,y0,x1,y1,color){
        for(let i = 0;i < y1-y0;i++){
            this.create_line(x0,y0+i,x1,y0+i,{r:color.r,g:color.g,b:color.b});
        }

        if (!this.boxes.some(box => box.x0 === x0 && box.y0 === y0 && box.x1 === x1 && box.y1 === y1)) {
            this.boxes.push({x0, y0, x1, y1, color, outline: false});
        }
        console.log("kotak", 1);
    }
